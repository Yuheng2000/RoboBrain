...
from model.llava_utils.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX
from train.llava_trainer import LLaVATrainer

from model.llava_utils import conversation as conversation_lib
from model.language_model.llava_qwen import LlavaQwenForCausalLM, LlavaQwenConfig
from model.llava_utils.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token
from model.llava_utils.utils import rank0_print, process_video_with_pyav, process_video_with_decord
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                ...
                if len(image_file) > 1 and len(image_file) < 9:
                    image = [self.process_image(f, "pad") for f in image_file]
                    image = [[im[0], im[1], "image"] for im in image]
                elif len(image_file) > 8:
                    image = [self.process_image(f, "pad") for f in image_file]
